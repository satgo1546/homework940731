<template>
  <div>
      ...
      <el-table :data="state.rows" border row-key="id">
        ...
        <el-table-column label="图片" width="140">
          <template v-slot="{ row, $index }">
            <img :src="'http://localhost:40731/' + row.filename" width="100" height="100" @click="dialogState.visible = true, dialogState.index = $index">
          </template>
        </el-table-column>
    ...
    <el-dialog
      :title="'图片 ID ' + state.rows[dialogState.index].id"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-checkbox v-model="state.rows[dialogState.index].visible" @change="updateRecord({ id: state.rows[dialogState.index].id, visible: state.rows[dialogState.index].visible })" label="审核通过" border></el-checkbox>
